"""
Sod shock tube — throughput + GPU memory benchmark (all 4 backends, N-sweep).
...
Saves:
  ...
  sod_memory.png    — peak GPU memory vs N (log-log)
...
def main():
    ...
    ax.set_title("Sod  —  GPU memory vs grid size\n(XLA_PYTHON_CLIENT_PREALLOCATE=false)", fontsize=12, pad=10)
    ax.legend(fontsize=11); ax.grid(True, which="both", lw=0.4, alpha=0.5)
    ax.set_xticks(all_N); ax.set_xticklabels([f"{n:,}" for n in all_N], rotation=35, ha="right", fontsize=8)
    if not has_mem:
