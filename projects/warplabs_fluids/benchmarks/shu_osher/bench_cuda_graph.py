"""
Warp CUDA-graph vs baseline vs JaxFluids throughput — Shu-Osher shock-density.
...
N_STEPS     = 200
N_BENCH     = 3
MAX_WALL_S  = 900.0       # Shu-Osher N=4096 JaxFluids needs ~674s
...
def bench_warp(N, scheme):
    dt    = _stable_dt(N)
    Q0, _ = shu_ic(N, GAMMA)
    solver = WarpEuler1D(N, L/N, gamma=GAMMA, bc="outflow", device="cuda", scheme=scheme)
...
from pathlib import Path
with tempfile.TemporaryDirectory(prefix=f"jxf_shucg_{N}_") as td:
    td = Path(td)
...
def main():
    ...
    try:
        ...
        tmp_dir = Path(tempfile.mkdtemp(prefix="cg_shu_"))
        ...
        jxf_ok = True
        print(f"[info] JaxFluids loaded from {case_json.name} (fp32+fp64, logging=NONE)")
    ...
    subtitle = (
        "1-D Euler  ·  Shu-Osher  ·  WENO5-Z+HLLC+RK3  ·  3 runs median\n"
    ...
    ax.set_title("Shu-Osher  —  Warp CUDA Graph vs Baseline vs JaxFluids", fontsize=13, pad=10)
